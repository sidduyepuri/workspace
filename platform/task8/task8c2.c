...
static int number; 
static struct kobject *files[MAX_FILES]; 
...
{ 
	...
	for (i = 0; i < MAX_FILES; i++) { 
		if (files[i]) { 
			if (file_attrs[i]) { 
				sysfs_remove_file(files[i], &file_attrs[i]->attr); 
	...
	for (i = 0; i < number; i++) { 
		...
		files[i] = kobject_create_and_add(filename, kernel_object); 
		if (!files[i]) 
			return -ENOMEM; 
