...
static ssize_t device_read(struct file *file ,char __user *user_buffer,size_t read_count,loff_t *offset)
{
	...
	//char kernel_buffer[100]="kernel";
	//int retval;
	retval = copy_to_user(user_buffer,kernel_buffer,read_count);
	user_buffer[read_count]='\0';
	pr_info("%s: copy to user returned:%d\n",__func__,retval);
	pr_info("%s:Kernel buffer:%s \t Count:%lu \t offset:%llu\n",__func__,kernel_buffer, read_count, *offset);
	return read_count;
}
...
static ssize_t device_write(struct file *file ,const char __user *user_buffer,size_t write_count,loff_t *offset)
{
	...
	retval= copy_from_user(kernel_buffer ,user_buffer, write_count);
	kernel_buffer[write_count]='\0';
	pr_info("%s:copy from user returned:%d\n",__func__,retval);
	pr_info("%s:Kernel Buffer:%s\t count:%lu \t offset:%llu\n",__func__,kernel_buffer,write_count,*offset);
